...
RegisterNetEvent('syko-taco:server:makeBeefTaco', function()
...
RegisterNetEvent('syko-taco:server:makeChickenBurrito', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    Player.Functions.RemoveItem("tortillas", 1)
...
RegisterNetEvent('syko-taco:server:makeQuesadillas', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    Player.Functions.RemoveItem("tortillas", 1)
...
RegisterNetEvent('syko-taco:server:makeNachos', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
...
RegisterNetEvent('syko-taco:server:makeCokeSoda', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
...
RegisterNetEvent('syko-taco:server:makeFanta', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    Player.Functions.RemoveItem("carbonated-water", 1)
    Player.Functions.RemoveItem("sugar-cubes", 1)
...
QBCore.Functions.CreateUseableItem("beef-taco", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("syko-taco:client:Eat", source, item.name)
    end
end)
...
QBCore.Functions.CreateUseableItem("coke-soda", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("syko-taco:client:Drink", source, item.name)
    end
end)
...
RegisterNetEvent("syko-taco:server:billPlayer", function(playerId, amount)
