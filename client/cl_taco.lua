...
function TacoZones()
	...
	exports['qb-target']:AddTargetEntity(jobPed, { 
		...
		{ 
		    type = "client",
		    event = "syko-taco:client:jobGarage",
		    icon = "fa-solid fa-clipboard-check",
		    label = "Take Out Delivery Van",
		    job = "taco"
		},
		{ 
		    type = "client",
		    event = "syko-taco:client:storeGarage",
		    icon = "fa-solid fa-clipboard-check",
		    label = "Store Vehicle",
		    job = "taco"
		},
...
RegisterNetEvent('syko-taco:client:jobGarage', function(vehicle)
...
RegisterNetEvent('syko-taco:client:storeGarage', function()
...
RegisterNetEvent("syko-taco:client:frontTray1", function()
...
RegisterNetEvent("syko-taco:client:jobFridge1", function()
...
RegisterNetEvent("syko-taco:client:ingredientStore", function()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "taco", Config.Items)
end)

RegisterNetEvent('syko-taco:clientToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
end)

RegisterNetEvent("syko-taco:bill", function()
    ...
    if bill ~= nil then
        ...
        TriggerServerEvent("syko-taco:server:billPlayer", bill.citizenid, bill.billprice)
    end
end)
